...
public class GenieToJson {
	...
	public void readGenieDataFile(String inputFilename, String outputFilename, String fileType,
			String sampleCancerTypeFilename) {
	...
	private void readAndWrite(FileReader fileReader, BufferedWriter writer, String fileType, String filename) {
		...
		try {
			...
			if (fileType.equals("clinical_sample")) {
				System.out.println(
						"Parsing clinical sample file. Please wait until the FINISHED indicator is presented....");
				...
				while ((line = reader.readLine()) != null) {
					...
					if (!line.startsWith("Hugo_Symbol")) {
						...
						if (lineElements[37].contains(":")) {
							...
							// String may have more than 1 : so best to just remove the bit up to the first
							// colon and the first colon
						...
						if (lineElements[9].equals("Nonsense_Mutation")) {
							jo.put("hgvspCodon",
									getCodonFromNonsenseMutation(lineElements[39], getProtposAsInteger(protpos)));
						} else if (lineElements[9].contains("Frame_Shift")) {
							jo.put("hgvspCodon",
									getCodonFromFrameshiftMutation(lineElements[39], getProtposAsInteger(protpos)));
						} else {
							jo.put("hgvspCodon", "");
						}
